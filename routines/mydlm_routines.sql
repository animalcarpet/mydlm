...
-- next however many
DROP PROCEDURE IF EXISTS `next_runtime` ; 
DELIMITER //
CREATE DEFINER='dlmadmin'@'localhost' PROCEDURE `next_runtime` (
  _job_id INTEGER UNSIGNED)
DETERMINISTIC
SQL SECURITY INVOKER 
CONTAINS SQL
BEGIN
  DECLARE _j_mi CHAR(24);
  DECLARE _j_hr CHAR(24);
  DECLARE _j_dm CHAR(24);
  DECLARE _j_mn CHAR(24);
  DECLARE _j_dw CHAR(24);
  DECLARE _c_mi CHAR(24);
  DECLARE _c_hr CHAR(24);
  DECLARE _c_dm CHAR(24);
  DECLARE _c_mn CHAR(24);
  DECLARE _c_dw CHAR(24);
--  DECLARE _inc INTEGER;


SELECT @debug;
  -- work out the increments
  SELECT `mi`,`hr`,`dm`,`mn`,`dw` INTO _j_mi, _j_hr, _j_dm, _j_mn, _j_dw
  FROM `mydlm`.`jobs`
  WHERE `job_id` = _job_id;

  SET _c_mi = CAST(MINUTE(NOW()) AS CHARACTER); 
  SET _c_hr = CAST(HOUR(NOW()) AS CHARACTER); 
  SET _c_dm = CAST(DAY(NOW()) AS CHARACTER); 
  SET _c_mn = CAST(MONTH(NOW()) AS CHARACTER); 
  SET _c_dw = CAST(DAYOFWEEK(NOW()) - 1 AS CHARACTER); -- ODBC standard differs from cron 

  IF _j_mi = '*' THEN

    IF @debug IS NOT NULL THEN
      SELECT CONCAT('Debug 1 - minute value = ', _j_mi );
    END IF;
    SET _j_mi = IF(CAST(MINUTE(NOW()) AS CHARACTER) = '59', '0', CAST(MINUTE(NOW()) + 1 AS CHARACTER));
  ELSEIF LOCATE(',',_j_mi) > 0 THEN  -- it's a set

    IF @debug IS NOT NULL THEN
      SELECT CONCAT('Debug 2 - minute value = ', _j_mi );
    END IF;

    mnloop:LOOP
      SET @test = SUBSTRING_INDEX(_j_mi,',',1);
      SET _j_mi =  SUBSTRING(_j_mi,LOCATE(',',_j_mi) + 1);

IF @debug IS NOT NULL THEN
  SELECT CONCAT('Debug 4 - minute value = ', _j_mi );
  SELECT CONCAT('Debug 5 - testing = ', @test );
END IF;

      IF CAST(@test AS UNSIGNED) < CAST(_c_mi AS UNSIGNED) THEN

IF @debug IS NOT NULL THEN
  SELECT CONCAT('Debug 6 - iterating = ', @test );
END IF;
  ...
  ELSE
    IF @debug IS NOT NULL THEN
      SELECT CONCAT('Debug 3 - minute value = ', _j_mi );
    END IF;
    SET _j_mi = IF(_j_mi = '59', '0', _j_mi);    
  END IF;

SELECT _j_mi;

  -- get NEXT minute  
--  SET _start = (UNIX_TIMESTAMP(DATE_FORMAT(DATE_ADD(NOW(),INTERVAL 1 MINUTE),'%Y-%m-%d %H:%i:00')));
  

/*
  loop

  IF _mn = * THEN -- its running every minute
   _inc = 60;
  ELSE
   IF MINUTE(NOW() <= _mn THEN
     _inc = (_mn - MINUTE(NOW()) * 60);
   ELSE
     _inc = (MINUTE(NOW()) )
   END
  END;
...
DROP PROCEDURE IF EXISTS `next_value`;
DELIMITER //
CREATE DEFINER='dlmadmin'@'localhost' PROCEDURE `next_value` (
  IN _vals CHAR(24),
  IN _test TINYINT UNSIGNED,
  IN _ro_at TINYINT UNSIGNED, -- LAST_DAY(NOW())
  IN _ro_val TINYINT UNSIGNED,
  OUT _selected TINYINT UNSIGNED)
DETERMINISTIC
SQL SECURITY INVOKER
BEGIN
  SET @this = NULL;
  ...
  IF _vals = '*' THEN
    IF @debug IS NOT NULL THEN
      SELECT CONCAT('Debug 1 - _test: ', _test, ' _vals: ', _vals);
    END IF;
    SET _selected = IF(_test = _ro_at, _ro_val, _test + 1);

  ELSEIF LOCATE(',',_vals) > 0 THEN  -- it's a set

    IF @debug IS NOT NULL THEN
      SELECT CONCAT('Debug 2 - _test: ', _test, ' _vals: ', _vals);
    END IF;
    -- store first value in case we loop past it
    SET @firstval = SUBSTRING_INDEX(_vals, ',', 1);
    
    myloop:LOOP
      SET @this = CAST(SUBSTRING_INDEX(_vals, ',', 1) AS UNSIGNED);
      SET _vals =  SUBSTRING(_vals, LOCATE(',', _vals) + 1);

      IF @debug IS NOT NULL THEN
        SELECT CONCAT('Debug 3 - testing: ', @this, ' against: ', _test);
      END IF;

      IF @this <= _test THEN
        IF @debug IS NOT NULL THEN
          SELECT CONCAT('Debug 4 - iterating');
        END IF;
  ...
  ELSE
    IF @debug IS NOT NULL THEN
      SELECT CONCAT('Debug 5 single val = ', _test);
    END IF;
    IF CAST(_vals AS UNSIGNED) + 1 = _ro_at THEN
      SET _selected = _ro_val;
    ELSE
      SET _selected = CAST(_vals AS UNSIGNED);
    END IF;
  END IF;

-- ET _selected = IF(_selected > _test,@firstval,_selected);
